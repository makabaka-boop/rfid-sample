...
app.get('/api/location-board', authMiddleware, (req, res) => {
  ...
  let hangSql = `SELECT h.*,
    ...
    (SELECT COUNT(*) FROM missing_part_notes m WHERE m.hang_id = h.id AND m.status != '已处理') as unresolved_missing_count,
    (SELECT MAX(ts) FROM (
      SELECT updated_at as ts FROM tags WHERE id = h.tag_id
      UNION ALL
      SELECT report_time as ts FROM anomaly_tickets WHERE hang_id = h.id AND status != '已关闭'
      UNION ALL
      SELECT report_time as ts FROM missing_part_notes WHERE hang_id = h.id AND status != '已处理'
      UNION ALL
      SELECT swap_time as ts FROM swap_records WHERE original_hang_id = h.id
      UNION ALL
      SELECT recover_time as ts FROM recovery_records WHERE hang_id = h.id
...
app.get('/api/location-board/export', authMiddleware, (req, res) => {
  ...
  if (onlyFree === 'true') {
    ...
    if (areaId) { areaSql += ' AND id = ?'; areaParams.push(Number(areaId)); }
    areaSql += ' ORDER BY floor, area_code';
    const areaList = db.prepare(areaSql).all(...areaParams);

    const freeHeaders = ['楼层', '区域名称', '区域编码', '层号', '位号', '状态', '区域容量', '区域空闲数'];
    const freeLines = [freeHeaders.join(',')];

    for (const area of areaList) {
      const hangs = db.prepare(`SELECT layer_no, position_no FROM hanging_records
        WHERE area_id = ? AND status IN (${ACTIVE_STATUSES.map(() => '?').join(',')})`)
        .all(area.id, ...ACTIVE_STATUSES);
      const occupiedSet = new Set(hangs.map(h => `${h.layer_no}_${h.position_no}`));
      const defaultLayers = Math.max(1, Math.ceil(area.capacity / POSITIONS_PER_LAYER));
      const maxPos = hangs.length > 0 ? Math.max(...hangs.map(h => h.position_no), POSITIONS_PER_LAYER) : POSITIONS_PER_LAYER;
      const cols = Math.max(POSITIONS_PER_LAYER, maxPos);
      const areaFree = [];
      for (let l = 1; l <= defaultLayers; l++) {
        for (let p = 1; p <= cols; p++) {
          if (!occupiedSet.has(`${l}_${p}`)) areaFree.push({ layerNo: l, positionNo: p });
        }
      }
      const freeCount = Math.max(0, area.capacity - hangs.length);
      areaFree.slice(0, freeCount).forEach(fp => {
        freeLines.push([
          `${area.floor}楼`,
          `"${area.area_name || ''}"`,
          area.area_code || '',
    ...
    const freeCsv = '\uFEFF' + freeLines.join('\n');
    const freeFilename = `空闲库位明细_${dayjs().format('YYYYMMDDHHmmss')}.csv`;
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="location_board_free.csv"; filename*=UTF-8''${encodeURIComponent(freeFilename)}`);
    return res.send(freeCsv);
  }
  ...
  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
  res.setHeader('Content-Disposition', `attachment; filename="location_board.csv"; filename*=UTF-8''${encodeURIComponent(filename)}`);
  res.send(csv);
});
