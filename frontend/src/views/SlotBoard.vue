<template>
  <div class="page-container">
    ...
    <div v-loading="loading">
      ...
      <div v-for="area in boardData.areas" :key="area.id" class="area-card">
        <div class="area-header">
          ...
          <div class="area-stats">
            ...
            <span class="stat-pill stat-free">空闲 <b>{{ area.free_count }}</b></span>
            <span class="stat-pill stat-swap">待调换 <b>{{ area.pending_swap_count }}</b></span>
            <span class="stat-pill stat-recovery">待回收 <b>{{ area.pending_recovery_count }}</b></span>
            ...
            <span class="occupancy-rate">
              占用率
              <el-progress :percentage="Math.round(area.occupancy_rate * 100)" :stroke-width="10" :show-text="false" style="width:120px; display:inline-block; margin:0 8px;" :color="occupancyColor(area.occupancy_rate)" />
              ...
              >
                ...
                <template v-if="slot.occupied && slot.hang">
                  <div class="slot-body">
                    ...
                    <div v-if="slot.hang.unresolved_missing_count > 0 || slot.hang.open_anomaly_count > 0" class="slot-alerts">
                      <el-tag v-if="slot.hang.unresolved_missing_count > 0" type="danger" size="small">缺件{{ slot.hang.unresolved_missing_count }}</el-tag>
                      <el-tag v-if="slot.hang.open_anomaly_count > 0" type="warning" size="small">异常{{ slot.hang.open_anomaly_count }}</el-tag>
                    </div>
                  </div>
                </template>
...
import { Search, Refresh, Plus, Tickets, CollectionTag, User, Clock, Calendar, Grid, ShoppingCartFull, Warning, Bell, Switch } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { getSlotBoardApi, getAreasApi } from '@/api'

const router = useRouter()
const route = useRoute()
const loading = ref(false)
const boardData = reactive({ summary: {}, areas: [], filters: { floors: [], responsible_persons: [] } })
const areasAll = ref([])
...
const summaryCards = computed(() => {
  ...
  return [
    ...
    { label: '空闲库位', value: s.total_free || 0, color: '#409eff', icon: Plus, click: null },
  ...
  if (slot.occupied && slot.hang) {
    ...
    router.push({
      ...
      query: {
        ...
        presetLayerNo: slot.layer_no,
        presetPositionNo: slot.position_no,
        autoOpen: '1'
      }
    })
  } else {
